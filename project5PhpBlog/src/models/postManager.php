...
class PostManager extends DatabaseManager
{
    ...
    function getPosts($keyword)
    {
        ...
        if (!empty($keyword)) {
            $statement = $db->prepare('SELECT post.id, title, category, chapo, user_name, content, DATE_FORMAT(post.date_create,"%D %b %Y") AS creation_date FROM `post`JOIN  `user`WHERE USER.id = `post`.`user_id`AND content like :keyword GROUP BY post.id
            ORDER BY post.date_create DESC LIMIT 0, 5');
            $statement->bindValue(":keyword", "%$keyword%");
            $statement->execute();
        } else {
            $statement = $db->prepare('SELECT post.id, title, category, chapo, user_name, content, DATE_FORMAT(post.date_create, "%D %b %Y") AS creation_date FROM `post` JOIN `user` WHERE USER.id = `post`.`user_id` GROUP BY post.id ORDER BY post.date_create DESC LIMIT 0, 5');
            $statement->execute();
        }
